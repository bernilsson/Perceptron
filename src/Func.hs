...
-- | readImages reads the contents of the input file and returns a list of images
--   (represented as a list of Doubles)
...
readImages input =
    ...
    let commentsEtc l = not(null l || head l == '#' || isAlpha (head l))
        processed = map read $ concatMap words $ tail $ filter commentsEtc $ lines input
...
readAnswers inputAnswers =
    map read $ filter (\line -> not (isAlpha (head line))) $ concatMap words $ removeComments (lines inputAnswers)
    where removeComments xs =  filter (\line -> not (null line || (head line) == '#')) xs

-- | writePGM converts a image (or weight list) to pgm-format and writes it to the given filename
writePGM :: FilePath -> [Double] -> IO ()
writePGM filename image = let ints = map (round . (1000*)) image :: [Int]
...
-- | rotImgCorrect tries to rotate the image so that the eyes are up and the mouth is down.
rotImgCorrect :: Int -> Image -> Image
rotImgCorrect width img = let images = take 4 (iterate (rotate90 width) img)
...
-- | updateWeights is used by batchTrain to modify the weights. This is basically the
...
-- | train takes one list of image and answer pairs to train on and one list to test on.
--   train returns trained weights
...
-- | trainHelper takes one list of image and answer pairs to train on, one to test on, a list of weights
--   and a tuple. The tuple should consist of the best weights known yet, and how many of the images
--   in the test list that was correctly classified with those weights. train returns trained
trainHelper :: [(Image,Double)] -> [(Image,Double)] ->  [Weight] -> (Int, [Double]) -> IO [Double]
trainHelper xs tests weights best =
    let (newWeights, errs) = batchTrain xs weights learnRate 0
        ...
        --traceMsg = "corr " ++ (show $ numCorTests) ++ " errs " ++ (show errs) ++ " best " ++ (show $ fst best)
...
-- | batchTrain takes a list of image and answer pairs and the weights to be trained.
--   It returns the trained weights.
...
batchTrain ((img,ans):xs) weights learnRate numErr =
    ...
    in batchTrain xs newWeights learnRate (if abs error > 0.5 then numErr+1 else numErr)
...
-- | percieveFace tries to classify a given image with a set of pretrained weights.
--   returns the face as an Int according to specs.
...
-- | percieve takes a image, weights and and a activation function, and returns the neurons output
...
-- | kFold does kFold validation on the given input. It returns the accuracy as a value between 0.0 and 1.0.
...
kFold k input = helper (cycle input) times 0
    where times = length input `div` k
          ...
          helper xs 0 totCorr = return $ fromIntegral totCorr / (fromIntegral $ length input)
          helper xs n totCorr =
              ...
              in do weights <- train trainData tests
                    ...
                    putStrLn $ "numCorrect " ++ show newTot ++ " rounds left " ++ show n
                    helper rest (n-1) newTot


numCorrect :: [(Image, Double)] -> [Weight] -> Int
numCorrect xs weights = sum $ map (\(img,ans) -> fromEnum $ percieve img weights step == ans) xs

test :: [(Image, Eyes)] -> [(Image, Mouth)] -> [Weight] -> [Weight] -> [(Face,Face,Bool)] -> ([(Face,Face,Bool)], Int)
...
test ((image,eye):eyes) ((_, mouth):mouths) eyew mouthw xs =
    let aj = (percieve image eyew step)
        ...
        in test eyes mouths eyew mouthw (((getFace aj moth),(getFace eye mouth),correct):xs)

-------------------------------------------------------------------
-------- Helper functions -----------------------------------------
-------------------------------------------------------------------

-- | chunks splits the list xs into chunks with length n (the last chunk may be shorter).
chunks :: Int -> [a] -> [[a]]
chunks _ [] = []
